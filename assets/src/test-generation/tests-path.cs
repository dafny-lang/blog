// Dafny program tests-path.dfy compiled into C#
...
// Command-line arguments: test tests-path.dfy
// tests-path.dfy

method {:verify false} {:main} _Test__Main_(_noArgsParameter: seq<seq<char>>)
{
  var success: bool := true;
  print @""UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.Test0: "";
    [[ try { ]]
{
      Test0()
      {
...
"";
  ...
  print @""UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.Test1: "";
    [[ try { ]]
{
      Test1()
      {
...
"";
  ...
  print @""UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.Test2: "";
    [[ try { ]]
{
      Test2()
      {
...
module UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests {
  method {:test} Test0()
  {
    ...
    var chessPos1: Chess.ChessPos := Chess.Pos.Pos(row := 6, col := 2);
  ...
  method {:test} Test1()
  {
    var color0: Chess.Color := Chess.Color.White;
    var kind0: Chess.Kind := Chess.Kind.King(c := color0);
    var chessPos0: Chess.ChessPos := Chess.Pos.Pos(row := 2, col := 2);
    expect 0 <= chessPos0.row < 8 && 0 <= chessPos0.col < 8, ""If this check fails at runtime, the test does not meet the type constraints"";
    var piece0: Chess.Piece := Chess.Piece.Piece(kind := kind0, at := chessPos0);
    var color1: Chess.Color := Chess.Color.Black;
    var kind1: Chess.Kind := Chess.Kind.Knight(c := color1);
    ...
    var color3: Chess.Color := Chess.Color.Black;
    var kind3: Chess.Kind := Chess.Kind.Pawn(c := color3);
    var chessPos3: Chess.ChessPos := Chess.Pos.Pos(row := 4, col := 7);
    expect 0 <= chessPos3.row < 8 && 0 <= chessPos3.col < 8, ""If this check fails at runtime, the test does not meet the type constraints"";
    var piece3: Chess.Piece := Chess.Piece.Piece(kind := kind3, at := chessPos3);
    var color4: Chess.Color := Chess.Color.Black;
    var kind4: Chess.Kind := Chess.Kind.Pawn(c := color4);
    var chessPos4: Chess.ChessPos := Chess.Pos.Pos(row := 4, col := 5);
    expect 0 <= chessPos4.row < 8 && 0 <= chessPos4.col < 8, ""If this check fails at runtime, the test does not meet the type constraints"";
    var piece4: Chess.Piece := Chess.Piece.Piece(kind := kind4, at := chessPos4);
    var piece5: seq<Chess.Piece> := [piece0, piece1, piece2, piece3, piece4];
    var validBoard0: Chess.ValidBoard := Chess.Board.Board(pieces := piece5);
  ...
  method {:test} Test2()
  {
    var color0: Chess.Color := Chess.Color.White;
    var kind0: Chess.Kind := Chess.Kind.King(c := color0);
    var chessPos0: Chess.ChessPos := Chess.Pos.Pos(row := 1, col := 2);
    expect 0 <= chessPos0.row < 8 && 0 <= chessPos0.col < 8, ""If this check fails at runtime, the test does not meet the type constraints"";
    var piece0: Chess.Piece := Chess.Piece.Piece(kind := kind0, at := chessPos0);
    var color1: Chess.Color := Chess.Color.Black;
    var kind1: Chess.Kind := Chess.Kind.Knight(c := color1);
    var chessPos1: Chess.ChessPos := Chess.Pos.Pos(row := 5, col := 6);
    expect 0 <= chessPos1.row < 8 && 0 <= chessPos1.col < 8, ""If this check fails at runtime, the test does not meet the type constraints"";
    var piece1: Chess.Piece := Chess.Piece.Piece(kind := kind1, at := chessPos1);
    var color2: Chess.Color := Chess.Color.Black;
    var kind2: Chess.Kind := Chess.Kind.Knight(c := color2);
    var chessPos2: Chess.ChessPos := Chess.Pos.Pos(row := 0, col := 3);
    expect 0 <= chessPos2.row < 8 && 0 <= chessPos2.col < 8, ""If this check fails at runtime, the test does not meet the type constraints"";
    var piece2: Chess.Piece := Chess.Piece.Piece(kind := kind2, at := chessPos2);
    var color3: Chess.Color := Chess.Color.Black;
    var kind3: Chess.Kind := Chess.Kind.Pawn(c := color3);
...
module Chess {
    ...
    decreases board
  {
    ...
    board.pieces[0].at != board.pieces[1].at &&
    board.pieces[0].at != board.pieces[2].at &&
    board.pieces[0].at != board.pieces[3].at &&
    board.pieces[0].at != board.pieces[4].at &&
    board.pieces[1].at != board.pieces[2].at &&
    board.pieces[1].at != board.pieces[3].at &&
    board.pieces[1].at != board.pieces[4].at &&
    board.pieces[2].at != board.pieces[3].at &&
    board.pieces[2].at != board.pieces[4].at &&
    ...
    decreases board, king, byPiece
  {
    match byPiece {
      case Knight(Black()) =>
        board.pieces[1].Threatens(king.at) || board.pieces[2].Threatens(king.at)
      case Pawn(Black()) =>
        board.pieces[3].Threatens(king.at) || board.pieces[4].Threatens(king.at)
      case _ /* _v0 */ =>
        false
    }
  }
    ...
    for row: int := 0 to 8 {
      for col: int := 0 to 8 {
        ...
        for n: int := 0 to |board.pieces| {
          ...
          image := image + ""_"" + match board.pieces[n].kind.c { case White() => ""white"" case Black() => ""black"" } + ""_"" + match board.pieces[n].kind { case King(_ /* _v1 */) => ""king"" case Knight(_ /* _v2 */) => ""knight"" case Pawn(_ /* _v3 */) => ""pawn"" };
        }
  ...
  type ValidBoard = board: Board
    | BoardIsValid(board)
    witness Board([Piece(King(White), Pos(0, 0)), Piece(Knight(Black), Pos(0, 1)), Piece(Knight(Black), Pos(0, 2)), Piece(Pawn(Black), Pos(0, 3)), Piece(Pawn(Black), Pos(0, 4))])
}
")]
...
namespace Chess {
  ...
  public partial class __default {
    ...
    {
      Chess._IKind _source0 = byPiece;
      {
        if (_source0.is_Knight) {
          Chess._IColor c0 = _source0.dtor_c;
          if (c0.is_Black) {
            return ((((board)).Select(BigInteger.One)).Threatens((king).dtor_at)) || ((((board)).Select(new BigInteger(2))).Threatens((king).dtor_at));
          }
        }
      }
      {
        if (_source0.is_Pawn) {
          Chess._IColor c1 = _source0.dtor_c;
          if (c1.is_Black) {
            return ((((board)).Select(new BigInteger(3))).Threatens((king).dtor_at)) || ((((board)).Select(new BigInteger(4))).Threatens((king).dtor_at));
          }
        }
      }
      {
    ...
    public static void Describe(Dafny.ISequence<Chess._IPiece> board)
    {
      ...
      if (Chess.__default.CheckmatedByPlayer(board, _0_whiteKing, Chess.Color.create_Black())) {
        if (!(Chess.__default.CheckedByPlayer(board, _0_whiteKing, Chess.Color.create_Black()))) {
          throw new Dafny.HaltException("chess.dfy(76,6): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("expectation violation").ToVerbatimString(false));}
  ...
  public partial class ValidBoard {
    private static readonly Dafny.ISequence<Chess._IPiece> Witness = Dafny.Sequence<Chess._IPiece>.FromElements(Chess.Piece.create(Chess.Kind.create_King(Chess.Color.create_White()), Chess.Pos.create(BigInteger.Zero, BigInteger.Zero)), Chess.Piece.create(Chess.Kind.create_Knight(Chess.Color.create_Black()), Chess.Pos.create(BigInteger.Zero, BigInteger.One)), Chess.Piece.create(Chess.Kind.create_Knight(Chess.Color.create_Black()), Chess.Pos.create(BigInteger.Zero, new BigInteger(2))), Chess.Piece.create(Chess.Kind.create_Pawn(Chess.Color.create_Black()), Chess.Pos.create(BigInteger.Zero, new BigInteger(3))), Chess.Piece.create(Chess.Kind.create_Pawn(Chess.Color.create_Black()), Chess.Pos.create(BigInteger.Zero, new BigInteger(4))));
    public static Dafny.ISequence<Chess._IPiece> Default() {
      return Witness;
    }
...
namespace UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests {
  ...
  public partial class __default {
    public static void Test0()
    {
      ...
      _2_chessPos0 = Chess.Pos.create(new BigInteger(7), BigInteger.Zero);
      if (!(((((_2_chessPos0).dtor_row).Sign != -1) && (((_2_chessPos0).dtor_row) < (new BigInteger(8)))) && ((((_2_chessPos0).dtor_col).Sign != -1) && (((_2_chessPos0).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(8,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      ...
      _6_chessPos1 = Chess.Pos.create(new BigInteger(6), new BigInteger(2));
      if (!(((((_6_chessPos1).dtor_row).Sign != -1) && (((_6_chessPos1).dtor_row) < (new BigInteger(8)))) && ((((_6_chessPos1).dtor_col).Sign != -1) && (((_6_chessPos1).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(13,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      ...
      if (!(((((_10_chessPos2).dtor_row).Sign != -1) && (((_10_chessPos2).dtor_row) < (new BigInteger(8)))) && ((((_10_chessPos2).dtor_col).Sign != -1) && (((_10_chessPos2).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(18,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      ...
      if (!(((((_14_chessPos3).dtor_row).Sign != -1) && (((_14_chessPos3).dtor_row) < (new BigInteger(8)))) && ((((_14_chessPos3).dtor_col).Sign != -1) && (((_14_chessPos3).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(23,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      ...
      _18_chessPos4 = Chess.Pos.create(new BigInteger(5), BigInteger.One);
      if (!(((((_18_chessPos4).dtor_row).Sign != -1) && (((_18_chessPos4).dtor_row) < (new BigInteger(8)))) && ((((_18_chessPos4).dtor_col).Sign != -1) && (((_18_chessPos4).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(28,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      ...
      if (!(Chess.__default.BoardIsValid(_21_validBoard0))) {
        throw new Dafny.HaltException("tests-path.dfy(32,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
    ...
    public static void Test1()
    {
      ...
      _1_kind0 = Chess.Kind.create_King(_0_color0);
      Chess._IPos _2_chessPos0;
      _2_chessPos0 = Chess.Pos.create(new BigInteger(2), new BigInteger(2));
      if (!(((((_2_chessPos0).dtor_row).Sign != -1) && (((_2_chessPos0).dtor_row) < (new BigInteger(8)))) && ((((_2_chessPos0).dtor_col).Sign != -1) && (((_2_chessPos0).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(39,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _3_piece0;
      _3_piece0 = Chess.Piece.create(_1_kind0, _2_chessPos0);
      Chess._IColor _4_color1;
      _4_color1 = Chess.Color.create_Black();
      ...
      _5_kind1 = Chess.Kind.create_Knight(_4_color1);
      Chess._IPos _6_chessPos1;
      _6_chessPos1 = Chess.Pos.create(BigInteger.One, new BigInteger(4));
      if (!(((((_6_chessPos1).dtor_row).Sign != -1) && (((_6_chessPos1).dtor_row) < (new BigInteger(8)))) && ((((_6_chessPos1).dtor_col).Sign != -1) && (((_6_chessPos1).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(44,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _7_piece1;
      _7_piece1 = Chess.Piece.create(_5_kind1, _6_chessPos1);
      Chess._IColor _8_color2;
      _8_color2 = Chess.Color.create_Black();
      ...
      _9_kind2 = Chess.Kind.create_Knight(_8_color2);
      Chess._IPos _10_chessPos2;
      _10_chessPos2 = Chess.Pos.create(BigInteger.One, new BigInteger(3));
      if (!(((((_10_chessPos2).dtor_row).Sign != -1) && (((_10_chessPos2).dtor_row) < (new BigInteger(8)))) && ((((_10_chessPos2).dtor_col).Sign != -1) && (((_10_chessPos2).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(49,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _11_piece2;
      _11_piece2 = Chess.Piece.create(_9_kind2, _10_chessPos2);
      Chess._IColor _12_color3;
      _12_color3 = Chess.Color.create_Black();
      ...
      _13_kind3 = Chess.Kind.create_Pawn(_12_color3);
      Chess._IPos _14_chessPos3;
      _14_chessPos3 = Chess.Pos.create(new BigInteger(4), new BigInteger(7));
      if (!(((((_14_chessPos3).dtor_row).Sign != -1) && (((_14_chessPos3).dtor_row) < (new BigInteger(8)))) && ((((_14_chessPos3).dtor_col).Sign != -1) && (((_14_chessPos3).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(54,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _15_piece3;
      _15_piece3 = Chess.Piece.create(_13_kind3, _14_chessPos3);
      Chess._IColor _16_color4;
      _16_color4 = Chess.Color.create_Black();
      ...
      _17_kind4 = Chess.Kind.create_Pawn(_16_color4);
      Chess._IPos _18_chessPos4;
      _18_chessPos4 = Chess.Pos.create(new BigInteger(4), new BigInteger(5));
      if (!(((((_18_chessPos4).dtor_row).Sign != -1) && (((_18_chessPos4).dtor_row) < (new BigInteger(8)))) && ((((_18_chessPos4).dtor_col).Sign != -1) && (((_18_chessPos4).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(59,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _19_piece4;
      _19_piece4 = Chess.Piece.create(_17_kind4, _18_chessPos4);
      Dafny.ISequence<Chess._IPiece> _20_piece5;
      _20_piece5 = Dafny.Sequence<Chess._IPiece>.FromElements(_3_piece0, _7_piece1, _11_piece2, _15_piece3, _19_piece4);
      ...
      _21_validBoard0 = _20_piece5;
      if (!(Chess.__default.BoardIsValid(_21_validBoard0))) {
        throw new Dafny.HaltException("tests-path.dfy(63,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess.__default.Describe(_21_validBoard0);
    }
    public static void Test2()
    {
      ...
      _1_kind0 = Chess.Kind.create_King(_0_color0);
      Chess._IPos _2_chessPos0;
      _2_chessPos0 = Chess.Pos.create(BigInteger.One, new BigInteger(2));
      if (!(((((_2_chessPos0).dtor_row).Sign != -1) && (((_2_chessPos0).dtor_row) < (new BigInteger(8)))) && ((((_2_chessPos0).dtor_col).Sign != -1) && (((_2_chessPos0).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(70,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _3_piece0;
      _3_piece0 = Chess.Piece.create(_1_kind0, _2_chessPos0);
      Chess._IColor _4_color1;
      _4_color1 = Chess.Color.create_Black();
      ...
      _5_kind1 = Chess.Kind.create_Knight(_4_color1);
      Chess._IPos _6_chessPos1;
      _6_chessPos1 = Chess.Pos.create(new BigInteger(5), new BigInteger(6));
      if (!(((((_6_chessPos1).dtor_row).Sign != -1) && (((_6_chessPos1).dtor_row) < (new BigInteger(8)))) && ((((_6_chessPos1).dtor_col).Sign != -1) && (((_6_chessPos1).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(75,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _7_piece1;
      _7_piece1 = Chess.Piece.create(_5_kind1, _6_chessPos1);
      Chess._IColor _8_color2;
      _8_color2 = Chess.Color.create_Black();
      ...
      _9_kind2 = Chess.Kind.create_Knight(_8_color2);
      Chess._IPos _10_chessPos2;
      _10_chessPos2 = Chess.Pos.create(BigInteger.Zero, new BigInteger(3));
      if (!(((((_10_chessPos2).dtor_row).Sign != -1) && (((_10_chessPos2).dtor_row) < (new BigInteger(8)))) && ((((_10_chessPos2).dtor_col).Sign != -1) && (((_10_chessPos2).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(80,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _11_piece2;
      _11_piece2 = Chess.Piece.create(_9_kind2, _10_chessPos2);
      Chess._IColor _12_color3;
      _12_color3 = Chess.Color.create_Black();
      ...
      _13_kind3 = Chess.Kind.create_Pawn(_12_color3);
      Chess._IPos _14_chessPos3;
      _14_chessPos3 = Chess.Pos.create(new BigInteger(6), new BigInteger(5));
      if (!(((((_14_chessPos3).dtor_row).Sign != -1) && (((_14_chessPos3).dtor_row) < (new BigInteger(8)))) && ((((_14_chessPos3).dtor_col).Sign != -1) && (((_14_chessPos3).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(85,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _15_piece3;
      _15_piece3 = Chess.Piece.create(_13_kind3, _14_chessPos3);
      Chess._IColor _16_color4;
      _16_color4 = Chess.Color.create_Black();
      ...
      if (!(((((_18_chessPos4).dtor_row).Sign != -1) && (((_18_chessPos4).dtor_row) < (new BigInteger(8)))) && ((((_18_chessPos4).dtor_col).Sign != -1) && (((_18_chessPos4).dtor_col) < (new BigInteger(8)))))) {
        throw new Dafny.HaltException("tests-path.dfy(90,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess._IPiece _19_piece4;
      _19_piece4 = Chess.Piece.create(_17_kind4, _18_chessPos4);
      Dafny.ISequence<Chess._IPiece> _20_piece5;
      _20_piece5 = Dafny.Sequence<Chess._IPiece>.FromElements(_3_piece0, _7_piece1, _11_piece2, _15_piece3, _19_piece4);
      ...
      _21_validBoard0 = _20_piece5;
      if (!(Chess.__default.BoardIsValid(_21_validBoard0))) {
        throw new Dafny.HaltException("tests-path.dfy(94,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("If this check fails at runtime, the test does not meet the type constraints").ToVerbatimString(false));}
      Chess.__default.Describe(_21_validBoard0);
    }
  }
} // end of namespace UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests
namespace _module {
  ...
  public partial class __default {
    public static void __Test____Main__(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> __noArgsParameter)
    {
      bool _0_success;
      _0_success = true;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.Test0: ")).ToVerbatimString(false));
      try {
        {
          UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.__default.Test0();
          {
...
")).ToVerbatimString(false));
      ...
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.Test1: ")).ToVerbatimString(false));
      try {
        {
          UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.__default.Test1();
          {
            Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"PASSED
")).ToVerbatimString(false));
          }
...
")).ToVerbatimString(false));
      ...
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.Test2: ")).ToVerbatimString(false));
      try {
        {
          UsersbhaveshbhatiablogblogassetssrctestgenerationchessdfyUnitTests.__default.Test2();
          {
            Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"PASSED
")).ToVerbatimString(false));
          }
...
")).ToVerbatimString(false));
      ...
      if (!(_0_success)) {
        throw new Dafny.HaltException("tests-path.dfy(1,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"Test failures occurred: see above.
").ToVerbatimString(false));}
    }
  }
