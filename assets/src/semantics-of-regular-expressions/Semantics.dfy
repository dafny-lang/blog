...
module Semantics {
    ...
    ensures Bisimilar(f(e), g(e))
  {
    ...
    UniqueCoalgebraHomomorphismHelper1(f, g, f(e), g(e));
  }

  lemma UniqueCoalgebraHomomorphismHelper1<A(!new)>(f: Exp -> Lang, g: Exp -> Lang, L1: Lang, L2: Lang)
    requires IsCoalgebraHomomorphism(f)
    requires IsCoalgebraHomomorphism(g)
    ...
    ensures Bisimilar(L1, L2)
  {
    forall k ensures Bisimilar#[k](L1, L2) {
      if k != 0 {
        UniqueCoalgebraHomomorphismHelper2(k, f, g, L1, L2);
      }
    }
  }

  lemma UniqueCoalgebraHomomorphismHelper2<A(!new)>(k: nat, f: Exp -> Lang, g: Exp -> Lang, L1: Lang, L2: Lang)
    requires IsCoalgebraHomomorphism(f)
    requires IsCoalgebraHomomorphism(g)
    ...
    ensures Bisimilar#[k](L1, L2)
  {
    ...
    if k != 0 {
      forall a ensures Bisimilar#[k-1](L1.delta(a), L2.delta(a)) {
        BisimilarityIsTransitiveAlternativeTwo(k-1, L1.delta(a),  f(e).delta(a), f(Delta(e)(a)));
        BisimilarityIsTransitiveAlternativeTwo(k-1, L2.delta(a),  g(e).delta(a), g(Delta(e)(a)));
        UniqueCoalgebraHomomorphismHelper2(k-1, f, g, L1.delta(a), L2.delta(a));
      }
    }
  }
